...
export function AlertsContextWrapper({children}: any){
    ...
    function setNewAlert(alertInfo: AlertInfoType){
        setAlertsInfo((current: any)=> [{id: genereteRandomId(),  ...alertInfo}, ...current])
    }

    return (<AlertsContext.Provider value={{alertsInfo, setNewAlert, removeAlert}}>{children}</AlertsContext.Provider>)
}